                    ...
                    >
                      {Array.from({
                        length: Math.min(
                          Math.ceil(options[item].length / 5),
                          3
                        ), // Limit columns to 3 max
                      }).map((_, colIndex) => (
                        <div
                          key={colIndex}
                          className={`grid ${
                            colIndex === 1 && options[item].length === 7
                              ? "grid-rows-5" // Leave space below if there are 7 items
                              : colIndex === 1 && options[item].length === 8
                              ? "grid-rows-5" // Leave space below if there are 8 items
                              : "grid-cols-1" // Default for balanced columns
                          } gap-2`}
                        >
                          {options[item]
                            .slice(
                              ...
                              colIndex === 1 &&
                                ...
                                ? colIndex * 5 + (options[item].length - 5) // For 7 or 8 items, put remaining in the second column
                                : colIndex * 5 + 5 // Otherwise, default split
                            )
                            .map((link: any, i: any) => (
                              <Link
                                key={i}
