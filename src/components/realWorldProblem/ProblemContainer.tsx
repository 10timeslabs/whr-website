...
// 					</div>
// 					{route !== "home" && <div>
// 						<div className='text-[64px] leading-[70px] max-[1000px]:hidden max-[1200px]:text-[48px] max-[1200px]:leading-[55px]'>Solutions</div>
// 						<div className='text-[var(--secondary-text-color)] text-[20px] max-[1000px]:hidden max-[1200px]:text-[16px]'>
// 							{route === "geo" ? geoSolutionData[currentIndex].subheading : gtmSolutionData[currentIndex].subheading}
// 						</div>
// 					</div>}
// 				</div>
...
// 					</div>
// 					{route === "home" && <div>
// 						<div className='text-[64px] leading-[70px] max-[1000px]:hidden max-[1200px]:text-[48px] max-[1200px]:leading-[55px]'>Wht, Whn,
// 							<span className='bg-gradient-to-r from-[#EE1CC082] via-[#7757DE] to-[#DD18FD40] bg-clip-text text-transparent'> Whr?</span>
// 						</div>
// 						<div className='text-[var(--secondary-text-color)] text-[20px] max-[1000px]:hidden max-[1200px]:text-[16px]'>Built with billions of signals, millions of sources, thousands of models to answer real questions
// 						</div>
// 					</div>}
...
const ProblemContainer = ({ cardData, route }: Props) => {
  ...
  const EngineScroll = dynamic(() => import("./EngineScroll"), { ssr: false });

  const [currentIndex, setCurrentIndex] = useState<number>(1);
  const pathName = usePathname();
  const handlePrev = () => {
    const length =
      route === "geo" ? geoSolutionData.length : gtmSolutionData.length;
  ...
  const geoSolutionData = [
    {
      heading: "Unified, Verified Data",
      subheading:
        "We consolidate events from multiple channels, clean them with AI+human checks, and deliver a single, trusted feed—no more guesswork or scattered listings",
      image: ResearchImage,
    },
    {
      heading: "Accurate Forecasting",
      subheading:
        "GEO aggregates upcoming & forecasted events, quantifies footfall, and helps you plan staffing, pricing, and inventory with confidence",
      image: TrendsImage,
    },
    {
      heading: "Real-Time Collaboration",
      subheading:
        "Receive instant updates on new or changing events. Coordinate via built-in trackers and notes, preventing chaos and capturing fresh business opportunities well ahead of time",
      image: CoordinateImage,
    },
    {
      heading: "Location-Focused Insights",
      subheading:
        "Define polygons or entire cities; filter by event size, type, inbound scores—so you can strategically deploy on-ground marketing and staffing for higher returns",
      image: TrackerImage,
    },
  ];
  const gtmSolutionData = [
    {
      ...
      subheading:
        "The right event isn’t always the biggest. Access enriched B2B event data with 20+ smart filters and proprietary scores—no more messy spreadsheets",
      image: ResearchImage,
    },
    {
      heading: "Co-ordinate Efficiently",
      subheading:
        "Simplify team and event planner communication with an all-in-one event management tool. Share event details, reduce confusion, and seamlessly coordinate participation, proposals, and negotiations—all in one dashboard",
      image: CoordinateImage,
    },
    {
      heading: "Promote Strategically",
      subheading:
        "Promote events with branded widgets, custom pages, and seamless asset integration. Engage prospects and share effortlessly with custom links",
      image: PromoteImage,
    },
    {
      heading: "Track Actively",
      subheading:
        "Track event participation, ROI, and feedback in a single dashboard. Manage reports, teams, and strategies seamlessly across geographies",
      image: TrackerImage,
    },
  ];
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center  relative">
      <Image
        src={TopgridImage}
        alt="grid"
        className="absolute top-0 -z-[10] max-[600px]:hidden"
      />
      <Image
        src={BottomgridImage}
        alt="grid"
      ...
      <div className="w-[100%] flex items-center justify-center relative">
        <div className="w-[87%] flex mt-[110px] justify-between max-[1000px]:flex-col">
          <div className="flex flex-col w-[25%] gap-8 max-[1000px]:w-[100%] max-[1000px]:gap-4">
            <div className="text-[64px] leading-[70px]">
              Real World Problems
            </div>
            <div className="text-[20px] text-[var(--secondary-text-color)]">
              A billion people go, do we know when, why, whr?
            </div>
            <div className="max-[1000px]:hidden">{MemoizedLottie}</div>
            {route !== "home" && (
              <div>
                ...
                <div className="text-[var(--secondary-text-color)] text-[20px] max-[1000px]:hidden max-[1200px]:text-[16px]">
                  {route === "geo"
                    ? geoSolutionData[currentIndex].subheading
                    : gtmSolutionData[currentIndex].subheading}
                </div>
              </div>
            )}
          </div>
          <div className="w-[60%] flex flex-col gap-4 max-[1000px]:w-[100%] max-[1000px]:justify-center">
            ...
            <div className="text-[var(--secondary-text-color)] text-[20px] hidden max-[1000px]:block">
              {route === "home"
                ? "Built with billions of signals, millions of sources, thousands of models to answer real questions"
                : route === "geo"
                ? geoSolutionData[currentIndex].subheading
                : gtmSolutionData[currentIndex].subheading}
            </div>
              ...
              <div className="w-full relative mt-10">
                {route === "geo" ? (
                  <div className="text-[40px] text-center font-semibold">
                    {geoSolutionData[currentIndex].heading}
                  </div>
                ) : (
                  <div className="text-[40px] text-center font-semibold">
                    {gtmSolutionData[currentIndex].heading}
                  </div>
                )}
                <div
                  className="absolute left-[-6%] top-[50%] w-[40px] h-[40px] flex items-center justify-center bg-white rounded-[40px] drop-shadow-[2px_4px_6px_rgba(0,0,0,0.20)] cursor-pointer max-[575px]:h-[25px] max-[575px]:w-[25px]"
                  onClick={handlePrev}
                >
                  <Image src={LeftArrow} alt="left" />
                </div>
                <Image
                  src={
                    route === "geo"
                      ? geoSolutionData[currentIndex].image
                      : gtmSolutionData[currentIndex].image
                  }
                  alt="banner"
                />
                <div
                  className="absolute right-[-6%] top-[50%] w-[40px] h-[40px] flex items-center justify-center bg-white rounded-[40px] drop-shadow-[2px_4px_6px_rgba(0,0,0,0.20)] cursor-pointer max-[575px]:h-[25px] max-[575px]:w-[25px]"
                  onClick={handleNext}
                >
                  <Image src={RightArrow} alt="right" />
                </div>
